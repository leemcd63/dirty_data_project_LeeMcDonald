...
Initially, I had written code to carry out each step above for each year's data. Taking into mind the DRY principle, and after much trial and error, I developed the below function for this purpose:
...
The function was then applied as below:
```{r eval=FALSE}
...
- candy_ratings containing rater_id, candy_name and ratings
...
Separating out the values within the other ratings was perhaps the trickiest part of this entire task due to the nature of the free text input, with a huge amount of inconsistency throughout. If I'd had the chance to revisit this task, my results could definitely be improved, however it would require a lot of time (and Regex)!

Similarly to my method of converting the data to tidy format, I originally had written code for both joy and despair ratings, but then managed to write a function to take care of this.
...
This function was then applied as below:
```{r eval=FALSE}
...
I then used bind_rows() to join the two sets with other candy ratings together. Once this was complete, it was time to recode the values in the candy_name column. This was not an exhaustive list, but it ended up being rather long. Given more time this could be tweaked for further accuracy. Any values which did not match the regex patterns in my case_when() arguments were changed to NA values, and then dropped.
```{r eval=FALSE}
...
The next step was to recode the values in countries, like above, I used case_when to replace values here. As this was a free text input there were multiple spellings for a country, in particular the USA and UK. I created variables to act as Regex patterns for USA, UK and for non-countries which would be "Not Specified". After this I used coalesce() to replace all NA values with "Not Specified".
...
The very last step in cleaning this data was recoding our candy_names, in particular removing non-candy values. I achieved this using the same method as above. After changing all non-candy items to NA, and tidying up the candy names using case_when, I used drop_na() to remove these ratings and then added a unique ID row for every rating in the data.
...
To answer this question I have dropped all rows with no rating, then summarised using n() to find the total number of rows. Note: Using drop_na() wasn't necessary here as the clean data has already had all NA values removed.
...
First I have dropped NA values in the age column, and filtered for distinct values of rater_id so we had one age per rater. Then I grouped by trick_or_treat and summarised to find the mean age per response to the trick or treat question. I've grouped these questions together as I thought it would be interesting to see the average age of those who did not respond to the question also.
```{r}
candy_data %>%
  drop_na(age) %>%
...
## Question 4
**For each of joy, despair and meh, which candy bar recieved the most of these ratings?**

First I have grouped by both rating and candy_name, then used summarise to count the number of each rating per candy using n(). After this I used slice_max() to pick the top result in each rating. Note: I have included the with_ties argument in the event of two candies having the same amount of ratings.
...
Using our new dataset with integer values as ratings, I've first grouped by gender and candy_name, then summarised using sum(rating) to calculate the total rating for each candy. After this I used slice_max() to pick the highest rated candy of each gender.
...
Using the same method as Question 6, I first grouped by year and candy_name, then summarised using sum(rating) to find the total_rating for each candy, and used slice_max() to pick the highest rated candy per year.
```{r}
candy_rating_int %>%
...
Similar to Question 6, "Any full-sized candy bar" is the highest rated for each year. If we filter it out from our results the answer is as follows:
```{r}
candy_rating_int %>%
  filter(candy_name != "Any full-sized candy bar") %>%
  group_by(year, candy_name) %>%
...
Now we can group by country and candy_name, summarise using sum(rating) and find the candy with the highest total_rating for each country using slice_max()
```{r}
candy_rating_int %>%
  ...
  slice_max(total_rating, n = 1, with_ties = TRUE)
```
As with Q6 and Q7, "Any full-sized candy bar" has taken the top spot for every country. Interestingly the UK has 4 tied results for the total_rating, a good example of why the with_ties argument in slice_max() is useful!
