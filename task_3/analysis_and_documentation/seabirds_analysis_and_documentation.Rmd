...
seabirds %>%
  ...
  summarise(total_sightings = n_distinct(record_id)) %>%
  ungroup() %>%
  slice_max(total_sightings)
```
...
```{r}
seabirds %>%
  filter(str_detect(species_common_name, regex("penguin", ignore_case = TRUE))) %>%
  summarise(number_of_species = n_distinct(species_common_name),
